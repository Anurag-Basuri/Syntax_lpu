...
const EventsGrid = ({ events = [], onEventClick }) => {
	if (!events?.length) return (
		<section aria-labelledby="arvantis-events">
			<h3 id="arvantis-events" className="section-title">Events</h3>
	...
	return (
		<section aria-labelledby="arvantis-events" id="events">
			<h3 id="arvantis-events" className="section-title mb-6">Events</h3>
							...
							>
								<div className="relative h-44 md:h-48 w-full overflow-hidden">
									...
									<div className="absolute left-4 top-4 chip mono">
										<Calendar size={12} /> <span className="ml-2">{formatDate(ev?.eventDate)}</span>
									</div>

									{/* diagonal type ribbon */}
									{ev?.type && <div className="ribbon">{ev.type}</div>}
								</div>

								<div className="p-5">
									<div className="flex items-start justify-between gap-3">
										<div className="min-w-0">
											<h4 id={`ev-${idKey}`} className="font-semibold text-lg truncate" style={{ color: 'var(--text-primary)' }}>{title}</h4>
									...
									<div className="mt-4 flex items-center gap-3 text-sm" style={{ color: 'var(--text-secondary)' }}>
										<span className="code-chip">{ev?.type || 'General'}</span>
										<span className="ml-auto mono muted">{ev?.location || ''}</span>
									</div>
								</div>
							</button>
						</motion.article>
