  ...
  return (
    <>
      <motion.section
        ...
        className="arvantis-hero relative overflow-hidden rounded-3xl min-h-[360px] md:min-h-[420px] flex items-center text-white"
      ...
      >
        ...
        <svg className="absolute inset-0 hero-grid" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden>
          <defs>
            <linearGradient id="g1" x1="0" x2="1">
              <stop offset="0" stopColor="var(--accent-1)" stopOpacity="0.06" />
        ...
        {/* Gentle gradient to ensure legibility */}
        ...
        <div className="relative z-10 max-w-6xl w-full mx-auto px-6 py-10 md:py-14 lg:py-16">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
            ...
            <div className="col-span-2">
              <motion.div initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.12 }}>
                ...
                <div className="flex items-center gap-4 mb-4">
                  <p className="hero-subtitle text-base md:text-lg max-w-3xl leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                    {fest?.description || 'A flagship celebration of technology, creativity and collaboration hosted by Syntax Club.'}
                  </p>

                  {/* Show small countdown if startDate is present */}
                  {fest?.startDate && (
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <a href="#events" className="btn-primary neon-btn" aria-label="Explore events">
                    Explore Events
                  </a>
                  {fest?.ticketsUrl && (
                    <a href={fest.ticketsUrl} target="_blank" rel="noreferrer" className="btn-ghost" aria-label="Buy tickets">
                      <Ticket size={16} /> Buy Tickets
                    </a>
                  )}
                  {fest?.sponsorContact && (
                    <a href={`mailto:${fest.sponsorContact}`} className="btn-ghost" aria-label="Sponsor contact">
                      Sponsor
                    </a>
                  )}
                </div>
            ...
            <div className="hidden lg:flex lg:justify-end">
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }} className="w-full max-w-sm rounded-2xl p-4 glass-card">
      ...
      {/* Sticky CTA floating button (appears on all pages) */}
      {fest?.ticketsUrl && (
        <div className="arv-sticky-cta" aria-hidden>
          <a className="cta-btn" href={fest.ticketsUrl} target="_blank" rel="noreferrer" title="Buy tickets">
            <Ticket size={16} /> <span className="hidden sm:inline">Get Tickets</span>
          </a>
          <a className="mini" href="#events" aria-label="Explore events">
            Explore
          </a>
        </div>
      )}
